...
describe('SecurityLevelPicker', () => {
  ...
  it('shows expanded detail rows only for the selected level', () => {
    ...
    expect(queryByText('Argon2id (64 MB memory, 3 iterations)')).toBeTruthy();

    rerender(<SecurityLevelPicker securityLevel={LEVEL_IDS[2]} onSelect={() => {}} />);
    expect(queryByText('Argon2id (64 MB memory, 3 iterations)')).toBeNull();
    expect(queryByText('Argon2id + ML-KEM-1024 hybrid via HKDF-SHA384')).toBeTruthy();
  });
