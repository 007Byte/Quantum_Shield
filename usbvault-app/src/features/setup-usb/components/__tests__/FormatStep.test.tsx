...
describe('FormatStep', () => {
  ...
  it('shows the default-name hint when vaultName is blank', () => {
    ...
    expect(getByText('Leave blank for default name (USBVault). Max 32 characters.')).toBeTruthy();
  });
  ...
  it('fires onChangePartitionName when the partition name input changes', () => {
    ...
    const { getByLabelText } = render(<FormatStep {...buildProps({ onChangePartitionName })} />);
  ...
  it('sanitizes and uppercases the partition name preview', () => {
    const { getByText } = render(<FormatStep {...buildProps({ partitionName: 'my-drive!!' })} />);
