...
describe('DeleteHistory', () => {
  it('renders the panel title without throwing', () => {
    const { getByText } = render(<DeleteHistory history={[]} panelStyle={{}} labels={labels} />);
  ...
  it('labels a secure-method entry with the secure wipe label', () => {
    const { getByText } = render(
      <DeleteHistory history={[populatedHistory[0]]} panelStyle={{}} labels={labels} />
    );
  ...
  it('labels a quick-method entry with the quick delete label', () => {
    const { getByText } = render(
      <DeleteHistory history={[populatedHistory[1]]} panelStyle={{}} labels={labels} />
    );
  ...
  it('applies the supplied panelStyle to the panel card', () => {
    const { getByText } = render(
      <DeleteHistory history={[]} panelStyle={{ marginTop: 24 }} labels={labels} />
    );
