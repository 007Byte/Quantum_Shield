...
# Find our own location (TOOLS partition)
...
open "http://127.0.0.1:$COMPANION_PORT"
...
while true; do
    ...
    # If we got here from the trap (cleanup), exit cleanly
    if ! kill -0 $$ 2>/dev/null; then
        break
    fi

    # Process exited — check if it was intentional (SIGTERM/SIGINT = 143/130)
