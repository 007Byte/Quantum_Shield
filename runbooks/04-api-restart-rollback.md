...
```bash
# Check pod status
kubectl get pods -n production -l app=qav-api -o wide

# View pod logs for errors
kubectl logs -n production deployment/qav-api --all-containers=true -f

# Check for panic messages
kubectl logs -n production deployment/qav-api --all-containers=true | grep -i "panic\|fatal\|error"

# Monitor resource usage
kubectl top pods -n production -l app=qav-api

# Check deployment status
kubectl describe deployment qav-api -n production

# View recent deployments
kubectl rollout history deployment/qav-api -n production

# Check service health endpoint
curl -s http://qav-api:8080/health | jq .
```
...
2. **Get pod status immediately**
   ...
   # Expected output:
   # qav-api-5f7d9c4b8d-abc12   0/1     CrashLoopBackOff   5
   # qav-api-5f7d9c4b8d-xyz78   1/1     Running             0
...
3. **Check recent deployment history**
   ...
   kubectl rollout history deployment/qav-api -n production --revision=3
   kubectl rollout history deployment/qav-api -n production --revision=2
   ```

4. **Quick health check**
   ```bash
   # Try direct port-forward to working pod (if any)
   kubectl port-forward -n production svc/qav-api 8080:8080 &
...
5. **Identify failure type**
   ...
   kubectl logs -n production deployment/qav-api --previous | tail -50
...
6. **Identify bad configuration**
   ```bash
   kubectl describe deployment qav-api -n production | grep -A 20 "Environment"

   # Or check configmap/secret
   kubectl get configmap qav-api-config -n production -o yaml
   kubectl get secret qav-api-secret -n production -o yaml

   # Check recent changes
   git log --oneline -10 -- k8s/production/qav-api-deployment.yaml
   ```

7. **Fix configuration**
   ```bash
   # Option 1: Edit deployment directly
   kubectl edit deployment qav-api -n production
   # Find the bad environment variable or config
   # Remove or correct it
   # Save and exit

   # Option 2: Edit configmap/secret
   kubectl edit configmap qav-api-config -n production
   # Fix values, save and exit

   # Option 3: Reapply from Git
   git checkout k8s/production/qav-api-deployment.yaml
   kubectl apply -f k8s/production/qav-api-deployment.yaml
   ```

8. **Monitor pod restart**
   ```bash
   watch -n 2 'kubectl get pods -n production -l app=qav-api'
...
9. **Rollback to previous working version**
   ...
   kubectl rollout undo deployment/qav-api -n production --to-revision=2

   # Or just rollback to immediately previous:
   kubectl rollout undo deployment/qav-api -n production

   # Monitor rollback progress
   kubectl rollout status deployment/qav-api -n production --timeout=5m
   ```

10. **Verify rollback success**
    ```bash
    # Check pod status
    kubectl get pods -n production -l app=qav-api

    # All pods should be Running and Ready: 1/1
    # CrashLoopBackOff should disappear

    # Check logs (should be clean)
    kubectl logs -n production deployment/qav-api --tail=20
...
12. **Delete pods forcefully**
    ...
    kubectl delete pod qav-api-5f7d9c4b8d-abc12 -n production --grace-period=0 --force

    # Or delete all at once:
    kubectl delete pods -n production -l app=qav-api --grace-period=0 --force
...
13. **Check circuit breaker status**
    ...
    curl -s http://qav-api:8080/metrics | grep -i circuit

    # Expected output shows circuit state:
    # qav_circuit_breaker_state{service="database"} 0    (closed, normal)
    # qav_circuit_breaker_state{service="redis"} 0       (closed, normal)
    # qav_circuit_breaker_state{service="s3"} 1          (open, using fallback)
...
14. **Verify health endpoint properly reflects circuit breaker state**
    ```bash
    curl -s http://qav-api:8080/health | jq '.circuit_breakers'
...
15. **Watch key metrics recover**
    ...
    kubectl exec -n production deployment/qav-api \
    ...
    kubectl logs -n production deployment/qav-api --all-containers=true \
...
16. **Verify traffic is returning to normal**
    ```bash
    # Check request rates
    kubectl logs -n production deployment/qav-api --tail=100 | grep -c "request"
...
17. **Test with real requests**
    ...
    curl -X POST http://qav-api:8080/api/v1/auth/login \
      -H "Content-Type: application/json" \
    ...
    curl -s http://qav-api:8080/api/v1/users | head -c 200
    ```
...
- [ ] All pods running and ready: `kubectl get pods -n production -l app=qav-api`
- [ ] No CrashLoopBackOff or error states
- [ ] Error rate <1%: Check Prometheus/Grafana
- [ ] P99 latency <200ms: Verify in metrics
- [ ] Health endpoint returns 200: `curl http://qav-api:8080/health`
- [ ] Sample API requests succeeding: Test 5 key endpoints
- [ ] Circuit breakers healthy (none stuck open): `curl http://qav-api:8080/metrics | grep circuit`
...
```bash
# Get current status
kubectl get deployment qav-api -n production
kubectl get pods -n production -l app=qav-api -o wide

# View logs
kubectl logs -n production deployment/qav-api -f
kubectl logs -n production deployment/qav-api --previous  # Logs from crashed pod
...
kubectl describe deployment qav-api -n production
kubectl describe pod qav-api-5f7d9c4b8d-abc12 -n production
