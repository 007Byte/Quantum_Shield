...
mod tests {
	...
	fn test_recover_with_different_share_subsets() {
		...
		let recovered1 = recover_secret(&vec![shares[0].clone(), shares[1].clone(), shares[2].clone()], 3).unwrap();
		let recovered2 = recover_secret(&vec![shares[2].clone(), shares[3].clone(), shares[4].clone()], 3).unwrap();
		let recovered3 = recover_secret(&vec![shares[0].clone(), shares[2].clone(), shares[4].clone()], 3).unwrap();
